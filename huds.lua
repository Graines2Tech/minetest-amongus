...
local function get_kill_cooldown_msg()
    ...
    local msg = "Kill available"
    if amongus.kill_current_cooldown == 0 then
        color = 0x00FF00
    else
        msg = msg .. " in " .. dump(amongus.kill_current_cooldown) .. "s"
    end
    return color, msg
end
...
function amongus.announce_meeting()
    ...
    local top = 0
    local length = 300
    local init = "Use the chat to discuss.\nRight click on the spawn block to vote.\n"
    for p_name, p in pairs(amongus.players) do
        if p.connected then
            table.insert(
                    ...
                    p.player:hud_add(
                        {
                            ...
                            position = POS_CENTER_MIDDLE,
                            offset = {x = 0, y = 0},
                            text = init .. tostring(amongus.meeting_delay) .. "s remaining.",
                            alignment = alignment,
                            scale = {x = length, y = 60},
                            number = 0x0000FF
                        }
                ...
                if p.connected then
                    ...
                    if hud_def ~= nil then
                        if hud_def.type == "text" then
                            p.player:hud_change(hud[2], "text", init .. tostring(timer_left) .. "s remaining.")
                        end
                    end
                end
            end
...
function amongus.announce_impostors()
    ...
    local announce = "There "
    if nb == 0 then
        announce = announce .. "are no impostors"
    elseif nb == 1 then
...
function amongus.announce_ghost(player_name)
    ...
    local announce = "You are now a ghost.\nYou can use fly and noclip modes.\n"
    if not amongus.is_impostor(player_name) and amongus.is_ghost(player_name) then
        announce = announce .. "Finish your tasks to help your team!"
    end
    local player = nil
